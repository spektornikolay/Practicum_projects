# Здесь собраны мои проекты и краткое описание к ним.
...
### Стек:
python, pandas, matplotlib, numpy, scipy, statsmodels, math 

### Выводы:
Выявлен характерный размер выборки для каждой группы и длительность эксперимента. Было выявлено, что внедрение нового алгоритма рекомендаций на рост ключевой метрики повлияло и повысило его на статистичекси значимую величину. Нововведение в приложение внедрять стоит. 
...
### Стек:
python, pandas, matplotlib, numpy, scipy, statsmodels 
...
### Стек:
python, pandas, matplotlib, numpy, scipy, statsmodels, os
...
### Стек:
python, pandas, matplotlib, seaborn, numpy, scipy, statsmodels 
...
•	знакомство с методом получения данных через API,;
•	знакомство с библиотекой Nominatim, позволяющей получить координаты для городов и дальнейший парсинг данных;
•	знакомство с базой данных SupaBase;
•	расширение своих знаний по аналитике данных, главным образом кодирование в Python  и создание дашборда в Yandex DataLense.
     
### Выводы
Был проделан анализ влияния погодных условий на ДТП в городах Пензе и Иркутске в декабре 2025 г.         
Конечным продуктом проекта является дашборд влияния погодных условий на ДТП в 2-х городах Пензе и Иркутске за декабрь 2025. Алгоритм работы был следующим: сначала я выцепил данные по городам России из Википедии через API, затем добавил к ним всем координаты через библиотеку Nominatim. Готовую таблицу с координатами я сохранил в базе данных SupaBase, затем я добавил данные по погодным условиям (максимальная и минимальная температуры, кол-во осадков, атмосферное давление, скорость ветра, влажность) по дням с сервиса Open Meteo также через API с помощью координат для Пензы и Иркутска. Таблицу с погодными данными для двух городов я сохранил в базу данных SupaBase. Далее я получил коды ОКАТО для всех регионов РФ с сайта ГИБДД в формате json. Затем с помощью функции get_dtp_cards я получил карточки с ДТП с пагинацией для Иркутска и Пензы по декабрю 2025. Отдельно для них я сделал по 4 таблицы для каждого из городов с данными по ДТП. Их я тоже сохранил в SupaBase. После формирования таблиц в SupaBase я подключил базу данных к Яндекс DataLense и собрал дашборд с двумя вкладками для каждого из городов. До этого для каждого из городов я создал по 2 датасета: с погодными условиями и с ДТП (в последнем я соединил все таблицы с ДТП по полю kart_id).        

### Стек:
python, pandas, Nominatim, warnings, time, logging, sys, API, Supabase. 
